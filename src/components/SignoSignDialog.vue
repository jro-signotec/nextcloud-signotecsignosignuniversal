		...
		<div class="send-modal-content">
			...
			<div class="external-label">
				...
				<NcTextField id="recipientField" ref="recipientRef" v-model="recipientEmail" :label-outside="true"
					type="email" required style="min-width: 400px" />
			</div>
			...
			<div class="confirm-modal__buttons">
				<NcButton variant="tertiary" :disabled="false" @click="onReject">
					{{ cancelLabel }}
				</NcButton>
...
export default {
	...
	computed: {
		dialogTitle(): string {
			return t('signotecsignosignuniversal', 'Send for signature')
		},
		...
		isSendDisabled(): boolean {
			...
			if (
				this.authType.value === 'password'
				&& this.password.trim() === ''
			) {
				return true
			}
			if (
				(this.authType.value === 'tan_sms'
					|| this.authType.value === 'tan_email')
				&& this.tanTarget.trim() === ''
			) {
				return true
			}
